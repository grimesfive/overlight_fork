...
#define ID_MENU_BOT			2
#define ID_MENU_NEW_VEHICLE_CHEATS			3
...
#define ID_CAR_DEFORMER 10
#define ID_CAR_SHOOTER 20


#define ID_BOT_ADD 10
#define ID_BOT_SPAWN 20
#define ID_BOT_SPAM 30
#define ID_BOT_DEF_CARS 40
...
static int menu_callback_new_vehicle_cheats(int op, struct menu_item *item)
{
	switch (op)
	{
	case MENU_OP_ENABLED:
		switch (item->id)
		{
		case ID_CAR_DEFORMER:
			return vehcheats.cars_deformer;
			break;
		case ID_CAR_SHOOTER:
			return vehcheats.vehicle_shooter;
			break;
		}
		break;
	case MENU_OP_SELECT:
		...
		{
		case ID_CAR_DEFORMER:
			vehcheats.cars_deformer ^= 1;
			break;
		case ID_CAR_SHOOTER:
			vehcheats.vehicle_shooter ^= 1;
			break;
		}
		break;
	}
...
static int menu_callback_bot(int op, struct menu_item *item)
{
	switch (op)
	{
	case MENU_OP_SELECT:
		...
		{
		case ID_BOT_ADD:
			g_pBotManager->AddBot();
			break;
		case ID_BOT_SPAWN:
			g_pBotManager->SpawnAllBots();
			break;
		case ID_BOT_SPAM:
			g_pBotManager->BotManagerSettings.bSpamDicks ^= 1;
			break;
		case ID_BOT_DEF_CARS:
			g_pBotManager->BotManagerSettings.bDeformerCars ^= 1;
			break;			
		}
	...
	case MENU_OP_ENABLED:
		...
		{
		case ID_BOT_SPAM:
			return g_pBotManager->BotManagerSettings.bSpamDicks;
			break;
		case ID_BOT_DEF_CARS:
			return g_pBotManager->BotManagerSettings.bDeformerCars;
			break;
		}
		break;
	}
...
void menu_maybe_init ( void )
{
	...
	struct menu *menu_main, *menu_cheats, *menu_bots, *menu_new_vehicle_cheats, *menu_cheats_mods, *menu_cheats_inv, *menu_cheats_money, *
	...
	*menu_netpatches_inrpc, *menu_netpatches_outrpc, *menu_netpatches_inpacket, *menu_netpatches_outpacket;
	...
	menu_bots = menu_new(menu_main, ID_MENU_BOT, menu_callback_bot);
	menu_new_vehicle_cheats = menu_new(menu_main, ID_MENU_NEW_VEHICLE_CHEATS, menu_callback_new_vehicle_cheats);
	...
	menu_item_add(menu_main, menu_bots, "Bots", ID_NONE, MENU_COLOR_DEFAULT, NULL);
	menu_item_add(menu_main, menu_new_vehicle_cheats, "New Vehicle Cheats", ID_NONE, MENU_COLOR_DEFAULT, NULL);
	...
	// MENU NEW VEHICLE CHEATS
	menu_item_add(menu_new_vehicle_cheats, NULL, "CAR DEFORMER", ID_CAR_DEFORMER, MENU_COLOR_DEFAULT, NULL);
	menu_item_add(menu_new_vehicle_cheats, NULL, "CAR SHOOTER", ID_CAR_SHOOTER, MENU_COLOR_DEFAULT, NULL);
	
	// MENU BOTS
	menu_item_add(menu_bots, NULL, "ADD BOTS", ID_BOT_ADD, MENU_COLOR_DEFAULT, NULL);
	menu_item_add(menu_bots, NULL, "SPAWN BOTS", ID_BOT_SPAWN, MENU_COLOR_DEFAULT, NULL);
	menu_item_add(menu_bots, NULL, "SPAM", ID_BOT_SPAM, MENU_COLOR_DEFAULT, NULL);
	menu_item_add(menu_bots, NULL, "DEFORM CARS", ID_BOT_DEF_CARS, MENU_COLOR_DEFAULT, NULL);
