...
void ChatOverLight()
{
	...
	static DWORD dwSuggTick = GetTickCount();

	if (GetTickCount() - dwSuggTick <= 10000)
	{
		char *buf = " Custom Chat ";
		float fxChatType = x_chat;
		float fyChatType = y_chat - chat_height - pD3DFontChat->DrawHeight() - 2;
		render->D3D_OL_Trapezium(fxChatType,
			fyChatType,
			pD3DFontChat->DrawLength(buf) + pD3DFontChat->DrawHeight(),
			pD3DFontChat->DrawHeight(),
			OLCheats->bShowOLChat ? OL_COLOR1_2(64) : OL_COLOR1(128));
		pD3DFontChat->Print(buf, OLCheats->bShowOLChat ? OL_COLOR2(64) : OL_COLOR2(255),
			fxChatType, fyChatType, false, false);
		fxChatType += pD3DFontChat->DrawLength(buf) + pD3DFontChat->DrawHeight() + 2;
		buf = " OL Chat ";
		render->D3D_OL_Parallelogram(fxChatType,
			fyChatType,
			pD3DFontChat->DrawLength(buf) + pD3DFontChat->DrawHeight(),
			pD3DFontChat->DrawHeight(),
			OLCheats->bShowOLChat ? OL_COLOR1_2(128) : OL_COLOR1(64));
		pD3DFontChat->Print(buf, OLCheats->bShowOLChat ? OL_COLOR2(255) : OL_COLOR2(64),
			fxChatType, fyChatType, false, false);
	}
	else
	{
		char *buf = " Press F7 to change chat. ";
		float fxChatType = x_chat + pD3DFontChat->DrawHeight();
		float fyChatType = y_chat - chat_height - pD3DFontChat->DrawHeight() - 2;
		render->D3D_OL_Parallelogram(fxChatType,
			fyChatType,
			pD3DFontChat->DrawLength(buf) + pD3DFontChat->DrawHeight(),
			pD3DFontChat->DrawHeight(),
			OL_COLOR1_2(128));
		pD3DFontChat->Print(buf, OL_COLOR2(255),
			fxChatType, fyChatType, false, false);

		if (GetTickCount() - dwSuggTick > 15000)
			dwSuggTick = GetTickCount();
	}
	...
	float RemoveImportantLenght = pFont_OL_Chat->DrawLength(RemoveImportantWord);
	D3DCOLOR alpha = 0xFF000000;
	float NickNameLength, TextLegth, MessageLenght, MessageHeight;
