...
static void Addini(const char *param_name, void * pParam, byte ini_type, char * szDefaultValue)
{
	struct ini_entry	*ent;

	if ((ent = ini_register_entry(param_name, ini_type)) != NULL)
		ini_register_data(ent, pParam, szDefaultValue);
}
static void ini_init ( void )
{
	...
	Addini("player_tags", &OLCheats->bPlayerTags, TYPE_BOOL, "true");
	Addini("vehicle_tags", &OLCheats->bVehicleTags, TYPE_BOOL, "true");
	Addini("chat_font_size", &OLCheats->ChatFontSize, TYPE_INT, "7");
	Addini("chat_font_name", &OLCheats->ChatFontName, TYPE_STRING, "Tahoma");
	Addini("small_font_size", &OLCheats->SmallFontSize, TYPE_INT, "6");
	Addini("small_font_name", &OLCheats->SmallFontName, TYPE_STRING, "Arial");
	Addini("footer_font_size", &OLCheats->FooterFontSize, TYPE_INT, "8");
	Addini("footer_font_name", &OLCheats->FooterFontName, TYPE_STRING, "Arial");
	//Addini("nickname_font_size", &OLCheats->NicknameFontSize, TYPE_INT, "8");
	//Addini("nickname_font_name", &OLCheats->NicknameFontName, TYPE_STRING, "Arial");
	Addini("menu_font_size", &OLCheats->MenuFontSize, TYPE_INT, "10");
	Addini("menu_font_name", &OLCheats->MenuFontName, TYPE_STRING, "Arial");
	Addini("use_dark_theme", &OLCheats->bUseDarkTheme, TYPE_BOOL, "false");
	Addini("remove_chat_background", &OLCheats->bTranspBackGroundChat, TYPE_BOOL, "false");
	Addini("OL_Chat_send_nickname", &OLCheats->bSendNickname, TYPE_BOOL, "true");
	Addini("OL_Chat_send_server", &OLCheats->bSendServer, TYPE_BOOL, "true");
	Addini("OL_DrawFinder", &OLCheats->bRenderFinder, TYPE_BOOL, "true");
	Addini("OL_ChangeGPCI", &OLCheats->bChangeGPCIa, TYPE_BOOL, "true");





	if ((ent = ini_register_entry("charset_fix", TYPE_INT)) != NULL)
		ini_register_data(ent, &set.charset_fix, "1");

	//if ((ent = ini_register_entry("chat_font_size", TYPE_INT)) != NULL)
	//	ini_register_data(ent, &set.chat_font_size, "6");
	//
	//if ((ent = ini_register_entry("chat_font_name", TYPE_STRING)) != NULL)
	//	ini_register_data(ent, set.chat_font_name, "Arial");
	...
	if ( (ent = ini_register_entry("i_have_edited_the_ini_file", TYPE_BOOL)) != NULL )
		ini_register_data( ent, &set.i_have_edited_the_ini_file, "true" );
	set.i_have_edited_the_ini_file = true;
	...
	*/
	...
	//if ( (ent = ini_register_entry("logo_png_filename", TYPE_STRING)) != NULL )
	//	ini_register_data(ent, set.logo_png_filename, "mod_sa_logo.png");
	//if ((ent = ini_register_entry("settings_png_filename", TYPE_STRING)) != NULL)
	//	ini_register_data(ent, set.settings_png_filename, "mod_sa_logo.png");
...
static void ini_entry_parse_type ( struct ini_entry *ent, int idx, const char *val )
{
	...
	case TYPE_BOOL:
		...
		if (_stricmp(split->argv[0], "true") == 0)
			* (bool *)ent_data->data = true;
		else if (_stricmp(split->argv[0], "false") == 0)
			* (bool *)ent_data->data = false;
		else
		{
...
void ini_load ( void )
{
	...
	char		filename[512], OL_FILENAME[512];;
	char		line[512], OL_line[512], line_copy[4096], *opt, *str, *val;
	FILE		*fd;
	int			len, i, j, match;
	int			array_idx;
...
parse_continue: ;
	...
	snprintf(OL_FILENAME, sizeof(OL_FILENAME), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, OL_INI_FILE);

	//Log("Loading %s...", filename);
	fd = fopen(OL_FILENAME, "r");

	if (fd == NULL)
	{
		//#### BE_SETTINGS.ini is uploaded in server in "bin" folder
		Log("Could not open %s, using default (or previous) settings.", OL_FILENAME);
		MessageBox(0, "I can't open OverLight_Settings.ini\nPlease put ALL mod files in GTA folder.",
			"Oh, sorry!", 0);
		abort();
		return;
	}
	...
	while ((fgets(OL_line, sizeof(OL_line), fd) != NULL))
	{
		len = (int)strlen(OL_line);
		...
		/* skip comments and empty lines */
		if (OL_line[0] == '#' || OL_line[0] == ';' || OL_line[0] == 0)
			continue;

		if (len > 0 && OL_line[len - 1] == '\\')
		{
			OL_line[len - 1] = ' ';
			strlcat(line_copy, OL_line, sizeof(line_copy));
			continue;
		}
		...
		if (*str == '[')
		{
			*str++ = 0;
			...
			if (*str != ']')	/* foo[] uses the next free array index; leave array_idx set to -1 */
			{
				...
				if (array_idx < 0)
				{
					Log("Parse error: %s (Negative array index)", OL_line);
					goto be_parse_continue;
				}

				if ((str = strchr(str, ']')) == NULL)
				{
					Log("Parse error: %s (missing ']')", OL_line);
					goto be_parse_continue;
				}
			}
		...
		else
		{
			*str++ = 0;
			...
			if (*str != '=')
			{
				Log("Parse error: %s (expected '=')", OL_line);
				goto be_parse_continue;
			}
		}
	...
	for (i = 0; i < ini_entry_count; i++)
	{
		...
		for (j = 0; j < ent->size; j++)
		{
			...
			{
				Log("Warning: setting '%s' was set multiple times in the mod_sa.ini file", ini_entry_name(ent, j));
			}
		...
		if (!isset)
			Log("Warning: setting '%s%s' was not present in the mod_sa.ini file", ent->name, (ent->size > 1) ? "[]" : "");
	}
